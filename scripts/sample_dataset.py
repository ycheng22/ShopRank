...
def compute_query_features(df):
    ...
    e_frac = (e_counts.reindex(total_counts.index).fillna(0) / total_counts).reset_index(name='e_fraction')
    ...
    cand_size['candidate_size_bin'] = np.where(cand_size['candidate_size'] >= 16, 'large', 'small')
    
    # Merge all
    q_df = query_texts[['query_id', 'query']].merge(
        query_texts[['query_id', 'len_bin']], on='query_id'
...
def print_distribution_report(df, split_name, total_queries=None):
    ...
    logger.info(f"--- Distribution Report for {split_name} ({total_queries} queries) ---")
    ...
    def report_dist(col, target_ratios):
        ...
        for val, target in target_ratios.items():
            ...
            logger.info(f"  {col}={val}: {actual:.1%} (target {target:.1%}, dev {dev:.1%})")
        return max_dev
...
def get_sample(q_df, n=1500):
    ...
    w = np.ones(len(q_df))
    w *= q_df['len_bin'].map(lambda x: target_len.get(x, 0) / (pool_len.get(x, 1e-5))).values
    w *= q_df['difficulty_bin'].map(lambda x: target_diff.get(x, 0) / (pool_diff.get(x, 1e-5))).values
    w *= q_df['candidate_size_bin'].map(lambda x: target_cand.get(x, 0) / (pool_cand.get(x, 1e-5))).values
...
def main():
    ...
    parser.add_argument("--examples", required=True, help="Path to shopping_queries_dataset_examples.parquet")
    parser.add_argument("--products", required=True, help="Path to shopping_queries_dataset_products.parquet")
    parser.add_argument("--out-dir", default="data", help="Output directory")
    parser.add_argument("--max-products", type=int, default=30000, help="Maximum number of unique products allowed")
    parser.add_argument("--dry-run", action="store_true", help="Do not write files, just print distribution")
    ...
    logger.info(f"Filtered examples from {initial_len} to {len(df_ex)} (small_version=1 & locale=us)")
    ...
    qrels = df_ex[df_ex['query_id'].isin(query_ids)][['query_id', 'product_id', 'esci_label']].copy()
    ...
    if len(unique_product_ids) > args.max_products:
        logger.error(f"Number of unique products ({len(unique_product_ids)}) exceeds --max-products ({args.max_products}).")
        logger.error("Stopping instead of dropping queries, as dropping would distort strata.")
        sys.exit(1)
        
    if max_dev > 0.02:
        logger.error(f"Maximum distribution deviation ({max_dev:.1%}) exceeds 2 percentage points!")
        sys.exit(1)
    ...
    products['product_text'] = products['product_title'] + " " + products['product_description']
    ...
    # Write splits
    columns_splits = ['query_id', 'text', 'locale', 'len_bin', 'difficulty_bin', 'has_complement', 'candidate_size_bin']
    ...
    products[['product_id', 'product_text']].to_parquet(out_dir / "products.parquet", index=False)
