...
def run_sample_dataset(out_dir):
    ...
    for line in output.split("\n"):
        if "Maximum distribution deviation" in line or "dev " in line or "has_complement deviation" in line:
            print(line)
...
def check_leakage(out_dir):
    ...
    train_q = pd.read_parquet(Path(out_dir) / "splits/train.parquet")['query_id'].tolist()
    ...
    if total_leaks == 0:
        ...
        print(f"Leakage check FAILED. Overlaps: train-dev({len(leak_train_dev)}), train-test({len(leak_train_test)}), dev-test({len(leak_dev_test)})")
...
def ingest_to_neon(out_dir):
    ...
    try:
        ...
        queries.to_sql("queries", engine, if_exists="append", index=False, method='multi', chunksize=1000)
        print(f"Inserting {len(qrels)} qrels...")
        qrels.to_sql("qrels", engine, if_exists="append", index=False, method='multi', chunksize=1000)
