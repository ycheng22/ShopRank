...
async def build_index(args, settings):
    ...
    for i in range(0, total_rows, batch_size):
        ...
        texts = (
            batch["product_text"]
            .apply(lambda t: truncate_text(t, args.max_len))
            .tolist()
        )
        ...
        records = [
            (pid, txt, str(emb.tolist()))
            for pid, txt, emb in zip(product_ids, texts, embeddings)
        ]
...
def main():
    ...
    parser.add_argument(
        "--batch-size", type=int, default=getattr(settings, "batch_size", 32)
    )
    parser.add_argument(
        "--max-len", type=int, default=getattr(settings, "max_len", 512)
    )
