...
这一节是整个项目最需要人工决策的地方，**配比与阈值已冻结，不要重新设计**。全随机抽样会让每一级提升被稀释到看不出来——ESCI 的 pair 级标签里 E 占 65%，典型查询用 BM25 就能答对，必须刻意提高困难样本的比例。

### 22.2.1 数据集变体（先冻结，否则下面所有数字失效）

**只使用 `small_version == 1` AND `product_locale == "us"`。**

- `small_version=1` 是 ESCI **Task 1（query-product ranking）的标准子集**，绝大多数论文据此报数，你的结果更有可比性。
- 变体不同，下面的分位数阈值就完全不同：同一个 40 分位在全量 US 集上是 `E-fraction ≤ 0.688`，在本变体上是 `≤ 0.312`。**这就是必须冻结变体的原因。**
- 该变体实测：**29,844 条查询**，含 C 标注的查询占 **25.8%**，每查询候选数中位数 **16**，候选 ≥20 的查询仅占 **19.5%**。

### 22.2.2 五个分层维度

| 分层维度 | 分箱（阈值已硬编码） | 目标配比与理由 |
| --- | --- | --- |
| 查询长度 | 短（1–2 词）/ 中（3–5）/ 长（6+） | **25 / 45 / 30**。长查询是稠密向量优势区，短查询是 BM25 优势区，两端都要有足够样本才能画出对比 |
| 标注难度 | 难 iff **E-fraction ≤ 0.312** | **40 / 60**。该阈值是本变体上的 40 分位，**硬编码、不得从抽样结果重算**。刻意超采困难样本是消融表能显示出提升的关键 |
| 互补品存在性 | 含 ≥1 条 C 标注 / 不含 | **至少 20% 含 C**。池子里有 25.8%，轻度超采即可达成。"搜手机壳返回手机"是电商检索最典型的错误，要有专门样本能测它 |
| 候选集规模 | 大 iff **候选数 ≥ 16**（中位数） | **40 / 60**。**阈值订正：原定 ≥20，但池子里只有 19.5% 的查询够格，40/60 不可达。**改用中位数切分，"大/小"相对本数据集定义，天然可达且不额外撑大商品数 |
| 语言 | 英语原生 / 中文翻译 / 法语翻译 | 主实验集全英语（`product_locale = us`）；另建两个平行的翻译查询集，商品库始终为英语（这样测的是跨语言检索） |

### 22.2.3 规模与切分

- **查询 1,500 条**（train 900 / dev 300 / test 300）。
- **为什么不是 4,000**：ESCI 的商品**几乎不跨查询共享**——实测 2,000 条查询去重后仍有 38,571 个商品，约 **19.3 unique products/query**。4,000 条查询会外推到 **73k 商品**，远超 30k 上限。
- **30,000 ÷ 19.3 ≈ 1,555**，故取 1,500 条，**让上限永不触发**。这比"设 4,000 条再让脚本丢弃"干净得多：一旦真丢掉 2,500 条，五维分层配比就被破坏，而脚本要跑完才会告诉你。
- 300 条 dev 按 IR 惯例完全够用（TREC Deep Learning 每年只用 43–54 个 topic 报数）。这一点要写进 README，因为读者会问。
- 切分 **60 / 20 / 20**，在五个维度上分层，**按 query 切而非按候选行切**，防止同一查询的候选跨切分泄漏。
- **`--max-products` 默认 30000，且在 1,500 条查询下不应生效。若它真的触发，脚本必须停下报错而非丢弃查询。**
- 起步 30k 的理由：0.5 GB 免费档要同时装下 768 维向量、HNSW 索引与 tsv 全文索引。索引建好后实测占用，确认有余量再考虑扩容，并在 `docs/DECISIONS.md` 留痕。
...
### 22.2.5 指标口径（写死在 `evals/metrics.py`，不要参数化）
...
### 22.2.6 检索范围与可比性声明（必须写进 README）

检索在**全部 30k 商品语料**上进行，而非仅在每条查询自己的候选集内重排。这更贴近真实工程场景，也是 recall@50 有意义的前提；代价是**召回到的未标注商品一律按不相关计**，绝对分数会被系统性压低。

叠加 22.2.2 的困难样本超采，README 的 Metrics & Conventions 一节必须写明：

> Retrieval runs over the full 30k corpus, and sampling deliberately over-represents hard cases (60% hard, ≥20% with Complement labels). Retrieved-but-unjudged products count as irrelevant. Absolute scores are therefore **not comparable** to ESCI leaderboard or reranking-task numbers; **only the deltas between rows of the ablation table are meaningful.**

**已知局限**：ESCI 的候选集本身不大（中位数 16），重排能发挥的空间有限，这会压低 M4 那一级的增益。写进 `LIMITATIONS.md`，不要等消融表出来才解释。

### 22.2.7 翻译集构造

用低价 provider 把 dev/test 的英语查询译为中文与法语，保留原 `query_id` 以便直接复用 qrels。README 明确标注为合成数据，仅用于跨语言召回评估，不用于报告绝对排序质量。
