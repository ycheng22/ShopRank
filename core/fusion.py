...
def fuse(runs: dict[str, list[ScoredHit]], method: Literal["rrf", "weighted", "none"], params: dict | None = None) -> list[ScoredHit]:
    if params is None:
        params = {}
    ...
    if method == "none":
        ...
        # Sort by fused_score
        sorted_pids = sorted(aggregated.keys(), key=lambda pid: aggregated[pid]["fused_score"], reverse=True)
        ...
        for name, hits in runs.items():
            ...
            normalized_runs[name] = {h.product_id: (h.raw_score - min_s) / range_s for h in hits}
