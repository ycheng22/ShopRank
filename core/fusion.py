...
class _AggregatedHit:
    def __init__(self, product_id: str) -> None:
        self.product_id = product_id
        self.bm25_score: float = 0.0
        self.dense_score: float = 0.0
        self.fused_score: float = 0.0
        self.hits_by_retriever: dict[str, ScoredHit] = {}
...
def fuse(
    ...
    aggregated: dict[str, _AggregatedHit] = {}
    for retriever_name, hits in runs.items():
        for hit in hits:
            if hit.product_id not in aggregated:
                aggregated[hit.product_id] = _AggregatedHit(hit.product_id)
    ...
    if method == "none":
        ...
        sorted_pids = sorted(
            aggregated.keys(),
            key=lambda p: aggregated[p].fused_score,
            reverse=True,
        )
        ...
        weights: dict[str, float] = params.get("weights", {"bm25": 0.5, "dense": 0.5})

        for pid, data in aggregated.items():
            weighted_score = 0.0
            for name in data.hits_by_retriever:
                norm_score = normalized_runs.get(name, {}).get(pid, 0.0)
